...
%     end
    ...
    I_x1y=0; I_x2y=0; I_x1x2=0; T = 0; Hx3gx2x1=0; Hx2gx1=0;
    ...
    for i=1:N
        for j=1:N
            if  m_ij(i,j)> eps && m_i(i)> eps
                %AEG note: problem is here, Hx2gx1 should be additive
                %within the i,j loop
                %Another option is to comptue H(X1,X2,X3 directly without
                %the multiple  conditional terms)
                Hx2gx1_add= m_ij(i,j)*log2(m_i(i)/m_ij(i,j));
                if Hx2gx1_add>0
                    Hx2gx1=Hx2gx1+Hx2gx1_add;
                end
            end
            for k=1:N
                m_ijk=pdf(i,j,k);                
                %AEG: here is where the 3d entropy needs to get computed H(X1,X2,X3) term
                %MAF: H(X1,X2,X3)= H(X1)+H(X2|X1)+H(X3|X2,X1)
                if m_ijk > eps && m_ij(i,j)> eps
                    Hx3gx2x1_add= m_ijk*log2(m_ij(i,j)/m_ijk);
                    if Hx3gx2x1_add>0
                        Hx3gx2x1=Hx3gx2x1+Hx3gx2x1_add;
                    end
                end
